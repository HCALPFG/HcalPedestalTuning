    ...
    private:
        ...
        class ChannelData {
            public:
                ...
                //long double capSum[4];
                //long int capEntries[4];
                double capSum[4];
                int capEntries[4];
        ...
        bool digiStart;
        // Tokens to read HBHE, HO, HF and CalibHits
        edm::EDGetTokenT<HBHEDigiCollection>      hbhedigisToken_; 
        edm::EDGetTokenT<HODigiCollection>        hodigisToken_; 
        edm::EDGetTokenT<HFDigiCollection>        hfdigisToken_; 
        edm::EDGetTokenT<HcalCalibDigiCollection> hcalcalibdigisToken_; 
        // Interval Of Validity of topology map
        int mapIOV_;
        // switch on/off pedestal tuning
        bool tunePedestals_;
        // add channels that do not exist in the new emap 
        bool addChannels_;
        // path to the old XML bricks
        string pathToOldXMLBricks_;
        // output directory for the new XML bricks
        string pathToNewXMLBricks_;
        // output directory for the final XML bricks
        string pathToFinalXMLBricks_;
...
HcalPedestalTuning::HcalPedestalTuning(const edm::ParameterSet& iConfig)
{
    ...
    hbhedigisToken_     = consumes<HBHEDigiCollection>((iConfig.getParameter<string>("digisName")));
    hodigisToken_       = consumes<HODigiCollection>((iConfig.getParameter<string>("digisName")));
    hfdigisToken_       = consumes<HFDigiCollection>((iConfig.getParameter<string>("digisName")));
    hcalcalibdigisToken_= consumes<HcalCalibDigiCollection>((iConfig.getParameter<string>("digisName")));
    mapIOV_             = iConfig.getParameter<int>("mapIOV");
    tunePedestals_      = iConfig.getParameter<bool>("tunePedestals");
    addChannels_        = iConfig.getParameter<bool>("addChannels");
    pathToOldXMLBricks_ = 
        iConfig.getUntrackedParameter<string>("pathToOldXMLBricks");
    pathToNewXMLBricks_ = 
        iConfig.getUntrackedParameter<string>("pathToNewXMLBricks");
    pathToFinalXMLBricks_ = 
        iConfig.getUntrackedParameter<string>("pathToFinalXMLBricks");
...
//            cout << "RBX : " << channelData.frontEndId.rbx();
//            cout << " " << channelData.capEntries[0]; 
//            cout << " " << channelData.capEntries[1]; 
//            cout << " " << channelData.capEntries[2]; 
//            cout << " " << channelData.capEntries[3]; 
//            cout << " " << channelData.frontEndId.rm(); 
//            cout << " " << channelData.frontEndId.qieCard(); 
//            cout << " " << channelData.frontEndId.adc() << endl;  // FIXME 
...
/* 
        map<uint32_t, ChannelData>::iterator ch_tmp;
        for (ch_tmp = channelList.begin(); ch_tmp != channelList.end(); ch_tmp++) 
        {
        cout << ch_tmp->second.frontEndId.rbx() << " :: "
                     << ch_tmp->first << " "
                     << ch_tmp->second.dacSetting << " "
                     //<< ch_tmp->second.getStability() << " "
                     << ch_tmp->second.capEntries[0] << " " 
                     << ch_tmp->second.capEntries[1] << " " 
                     << ch_tmp->second.capEntries[2] << " "
                     << ch_tmp->second.capEntries[3]  << " " 
                     << ch_tmp->second.getChannelPedestal() <<endl;
        } // FIXME
*/
    ...
    // get HCAL digis
    iEvent.getByToken(hbhedigisToken_,    hbheDigis);
    iEvent.getByToken(hodigisToken_,      hoDigis);
    iEvent.getByToken(hfdigisToken_,      hfDigis);
    iEvent.getByToken(hcalcalibdigisToken_,calibDigis);
    ...
    for(HBHEDigiCollection::const_iterator d = hbheDigis->begin(); 
        ...
        for (int ts = 0; ts < size; ts++) {
            ...
            //cout << d->id() << " " << ch->second.frontEndId.rbx() << " " << ts << " " << d->sample(ts).capid() << endl; // FIXME
        } 
/*
        cout << " fillHBHEDigiCollection :: " 
             << d->id() << " :: " 
             <<  rawId << " :: " 
             <<  ch->first << " :: " 
             //<< ts << " " 
             ...
             << ch->second.capSum[2] << " " 
             << ch->second.capSum[3] << " "  
             << ch->second.capEntries[0] << " " 
             << ch->second.capEntries[1] << " " 
             << ch->second.capEntries[2] << " " 
             << ch->second.capEntries[3] << endl;   // FIXME 
*/
    }
    // loop over HO digis
...
/*
        cout << " fillHcalCalibDigiCollection :: " 
             << d->id() << " :: " 
             //<< ts << " " 
             << ch->second.capSum[0] << " " 
             << ch->second.capSum[1] << " " 
...
/*
    map<uint32_t, ChannelData>::iterator ch_temp;
    for (ch_temp = channelList.begin(); ch_temp != channelList.end(); ch_temp++) {
        cout << ChannelData::evtCounter     << " " 
             << ch_temp->second.frontEndId.rbx() << " :: " 
             << ch_temp->second.dacSetting       << " " 
             //<< ch->second.getStability()   << " " 
             << ch_temp->second.getChannelPedestal() <<endl; 
        cout << " " << ch_temp->second.capEntries[0]; 
        cout << " " << ch_temp->second.capEntries[1]; 
        cout << " " << ch_temp->second.capEntries[2]; 
        cout << " " << ch_temp->second.capEntries[3];  
        cout << endl;
    } // FIXME  
*/
}
...
void HcalPedestalTuning::endJob() {
    ...
    writeOutput();
    // add channels that exist in Old XML, but not in the new XML
    if(addChannels_) addChannels();
    ...
    for(int i=0; i<nRBXes; i++) {
        ...
        if (brick) {
            ...
            while (data) {
                ...
                // To avoid seg fault caused by the case where a channel exists in the old XML
                // but not in the current emap, i.e., not in the channelList,
                // check if the fakeFrontEndId exists in rawIdMap
                    
                    //cout << atoi(data->Attribute("rm")) << endl;
                    //cout << atoi(data->Attribute("card")) << endl;
                    //cout << atoi(data->Attribute("qie")) << endl;
                    //cout << RBXes[i] << endl;
                    //cout << "fakeFrontEndId: " << fakeFrontEndId << endl; 
                    //cout << "fakeFrontEndId: " << fakeFrontEndId.rawId() << endl; 
                    //cout << "fakeFrontEndId.rbxIndex: " << fakeFrontEndId.rbxIndex() << endl; 
                    //cout << "fakeFrontEndId.rbx: " << fakeFrontEndId.rbx() << endl; 
                    //cout << "rawIdMap.count(fakeFrontEndId): " << rawIdMap.count(fakeFrontEndId) << endl;  // FIXME
                    
               
                if(rawIdMap.count(fakeFrontEndId)!=0) 
                //if(rawIdMap.find(fakeFrontEndId)!=rawIdMap.end()) 
                { 
                    num_ch++;
                ...
                {
                    cout << "This channel needs to be added to the bricks as it is :: "  
                         << RBXes[i]  
                         << " :: rm= " << atoi(data->Attribute("rm")) 
                         << " card= " << atoi(data->Attribute("card")) 
                         << " qie= " << atoi(data->Attribute("qie")) 
                         << endl;
                }
...
/*       
        cout << " tunePedestals :: " 
             << ch->second.frontEndId.rbx() << " :: " 
             << ch->second.dacSetting << " " 
             << ch->second.getStability() << " " 
             << ch->second.isHOX << " " 
             << ch->second.isSiPM << " " 
             << ch->second.isCalib << " " 
             << ch->second.getChannelPedestal() <<endl; //FIXME 
*/
        //if( (ch->second.getStability()==0.0)) continue; // || (ch->second.isSiPM && !ch->second.isCalib) ) continue;
        if( (ch->second.getStability()==0.0) || (ch->second.isCalib) ) continue;
...
void HcalPedestalTuning::writeOutput() {
    ...
    string outputDir;
    if( pathToNewXMLBricks_ == pathToOldXMLBricks_ ) {
        tagName_ = "uniform6Nov2013";
        outputDir = pathToNewXMLBricks_ + "/" + tagName_ + "_bricks/";
        boost::filesystem::create_directories
            (boost::filesystem::path(outputDir));

        cout<<">> ----> WARNING: 'pathToNewXMLBricks_' and "
    ...
    cout<<">> For the all HCAL channels listed in "
        <<("'" + run_string + "_all_channels.txt'").c_str() << endl;
...
void HcalPedestalTuning::writeUnstable(const string& fOutputDir) {
    ...
    ofstream fOut((fOutputDir + run_string + 
                "_missing_unstable_channels.txt").c_str());
    char buffer[1024];
    sprintf(buffer,
            "# %8s %8s %8s %8s %8s %8s %8s %8s %12s %8s "
            "%10s %10s %10s %10s %10s\n",
    ...
    cout<<">> For the missing HCAL channels listed in "
        <<("'" + run_string + "_missing_unstable_channels.txt'").c_str()
...
void HcalPedestalTuning::addChannels() 
{
    ...
    //for(int i=0; i<1; i++) {
        ...
        vector<int> rm_old, card_old, qie_old, dac_old;
        vector<int> rm_new, card_new, qie_new, dac_new;
        vector<int> rm_final, card_final, qie_final, dac_final;

        //    
        // Read old XML
        //    
        TiXmlDocument xmlIn_old((pathToOldXMLBricks_ + "/" + RBXes[i] + ".xml").c_str());
        bool xmlLoaded_old = xmlIn_old.LoadFile(); 
        if (!xmlLoaded_old) 
        {
            cout<<">> PROBLEM READING XML File:\n\t "
                <<(pathToOldXMLBricks_ + "/" + RBXes[i] + ".xml").c_str()<<"\n";
        ...
        {
            cout << ">> Reading the old xml file: " << (pathToOldXMLBricks_ + "/" + RBXes[i] + ".xml").c_str() << endl; 
        } 

        TiXmlElement *brick_old, *data_old;
        brick_old = xmlIn_old.FirstChildElement("CFGBrick");
        if (brick_old) {
            data_old = brick_old->FirstChildElement("Data");
            while (data_old) {

                num_ch_old++;
                rm_old.push_back(atoi(data_old->Attribute("rm")));
                card_old.push_back(atoi(data_old->Attribute("card")));
                qie_old.push_back(atoi(data_old->Attribute("qie")));
                dac_old.push_back(atoi(data_old->GetText()));

//                cout << RBXes[i] << " :: " 
//                     << atoi(data_old->Attribute("rm")) << " " << atoi(data_old->Attribute("card")) << " " 
//                     << atoi(data_old->Attribute("qie")) << " " << atoi(data_old->GetText()) << endl;  // FIXME

                data_old = data_old->NextSiblingElement("Data"); 
            }
        }
        else {
            cout<<">> PROBLEM: CFGBrick element not foundi in file:\n\t "
        ...
        // Read new XML
        // 
        TiXmlDocument xmlIn_new((pathToNewXMLBricks_ + "/" + RBXes[i] + ".xml").c_str());
        bool xmlLoaded_new = xmlIn_new.LoadFile(); 
        if (!xmlLoaded_new) 
        {
            cout<<">> PROBLEM READING XML File:\n\t "
                <<(pathToNewXMLBricks_ + "/" + RBXes[i] + ".xml").c_str()<<"\n";
            return;
        }
        else 
        {
            cout << ">> Reading the new xml file: " << (pathToNewXMLBricks_ + "/" + RBXes[i] + ".xml").c_str() << endl; 
        } 

        TiXmlElement *brick_new, *data_new;
        brick_new = xmlIn_new.FirstChildElement("CFGBrick");
        if (brick_new) {
            data_new = brick_new->FirstChildElement("Data");
            while (data_new) {

                num_ch_new++;
                rm_new.push_back(atoi(data_new->Attribute("rm")));
                card_new.push_back(atoi(data_new->Attribute("card")));
                qie_new.push_back(atoi(data_new->Attribute("qie")));
                dac_new.push_back(atoi(data_new->GetText()));

//                cout << RBXes[i] << " :: " 
//                     << atoi(data_new->Attribute("rm")) << " " << atoi(data_new->Attribute("card")) << " " 
//                     << atoi(data_new->Attribute("qie")) << " " << atoi(data_new->GetText()) << endl;  // FIXME

                data_new = data_new->NextSiblingElement("Data"); 
            }
        }
        else {
            cout<<">> PROBLEM: CFGBrick element not foundi in file:\n\t "
        ...
        //
        // Construct the final list of channels
        //
        // loop over old xml 
        for(unsigned int iold=0; iold<rm_old.size(); iold++)  
        { 
            // loop over new xml and check if this channel exists in the new xml
            int dac = -1;
            for(unsigned int inew=0; inew<rm_new.size(); inew++)  
            { 
                if( rm_old.at(iold) == rm_new.at(inew) &&
                    card_old.at(iold) == card_new.at(inew) &&
                    qie_old.at(iold) == qie_new.at(inew) )
                { 
                    dac = dac_new.at(inew);  
                    continue;
                }        
            ...
            rm_final.push_back(rm_old.at(iold));
            card_final.push_back(card_old.at(iold));
            qie_final.push_back(qie_old.at(iold));
            if(dac==-1) dac = dac_old.at(iold);
            dac_final.push_back(dac);
        }
        
        // loop over new xml 
        for(unsigned int inew=0; inew<rm_new.size(); inew++)  
        { 
            // loop over the final xml and check if this channel exists in the old xml
            int dac = -1;
            for(unsigned int iold=0; iold<rm_old.size(); iold++)  
            { 
                if( rm_new.at(inew) == rm_old.at(iold) &&
                    card_new.at(inew) == card_old.at(iold) &&
                    qie_new.at(inew) == qie_old.at(iold) )
                { 
                    dac = dac_old.at(iold);  
                    continue;
                }        
            ...
            if(dac==-1) 
            {
                rm_final.push_back(rm_new.at(inew));
                card_final.push_back(card_new.at(inew));
                qie_final.push_back(qie_new.at(inew));
                dac_final.push_back(dac); 
            }
        }
        
        cout << "#channels in old xml: " << dac_old.size() << endl; 
        cout << "#channels in new xml: " << dac_new.size() << endl; 
        cout << "#channels in final xml: " << dac_final.size() << endl; 
/*        
        // final 
        for(unsigned int ifinal=0; ifinal<rm_final.size(); ifinal++)   
        { 
            cout << RBXes[i] << "(final) :: " 
                 << rm_final.at(ifinal) << " " << card_final.at(ifinal) << " " 
                 << qie_final.at(ifinal) << " " << dac_final.at(ifinal) << endl; // FIXME 
        } 
*/
        // 
        // Write a new XML file 
        // 
        // put the channel info to ChannelData
        vector<ChannelData> channelData_final;
        for(unsigned int ifinal=0; ifinal<rm_final.size(); ifinal++)    
        { 
            HcalFrontEndId frontEndId = 
                HcalFrontEndId(RBXes[i],
                    rm_final.at(ifinal),
                    1, 1, 1,
                    card_final.at(ifinal),
                    qie_final.at(ifinal));
            ChannelData channelData_tmp;
            channelData_tmp.frontEndId = frontEndId;
            channelData_tmp.dacSetting = dac_final.at(ifinal);
            channelData_final.push_back(channelData_tmp);
        } 
        // sort by compareRBXInfo() 
        sort(channelData_final.begin(), channelData_final.end(), compareRBXInfo()); 
        string theRBX = RBXes[i]; 
        ...
        for(unsigned int ich=0;  ich<channelData_final.size(); ich++)    
        {
            int RM   = channelData_final.at(ich).frontEndId.rm();
            int CARD = channelData_final.at(ich).frontEndId.qieCard();
            int QIE  = channelData_final.at(ich).frontEndId.adc();
            int DAC  = channelData_final.at(ich).dacSetting;
            stringstream ss;
            ss << hex << DAC;
            TiXmlElement *data = new TiXmlElement("Data");
            data->SetAttribute("elements","1");
        ...
        xmlOut.SaveFile((pathToFinalXMLBricks_ + "/" + theRBX + ".xml").c_str());

    } //for(int i=0; i<nRBXes; i++)

    cout<< ">> Final XML bricks written out!\n";
